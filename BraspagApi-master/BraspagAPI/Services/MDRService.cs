...
namespace BraspagAPI.Services
{
    public class MDRService : IMDRService
    {
        ...
        public async Task<List<Adquirente>> GetMBR()
        {
            ...
            return await Task.Run(() => (Adquirentes)); 

             
        }

        public async Task<List<string>> ValidarTransacao(Transacao t)
        {
            ...
            return await Task.Run(() => (listaErros));
                
    }

        public async Task<Resposta> GetValorLiquido(Transacao t)
        {
            var adquirente = await Task.Run(()=>(GetMBR().Result.FirstOrDefault(x => x.Nome.Contains(t.Adquirente))));
            ...
            return await Task.Run(() => (Resposta)); 
        }
    }
}